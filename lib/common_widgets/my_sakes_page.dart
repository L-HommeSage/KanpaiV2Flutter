...
class MySakesPage extends StatefulWidget {
  const MySakesPage({this.database, this.user});
...
class _MySakesPageState extends State<MySakesPage> {
  List<Sake> sakes = [];
  bool loading = false;
  ...
  Future<List<Sake>> _loadMySakes(User user) async {
    print("called");
    print(user.sakeList);
    for (int i = 0; i < user.sakeList.length; i++) {
      await Firestore.instance
          .collection("sakes")
          .document(user.sakeList[i])
          .get()
          .then((value) {
